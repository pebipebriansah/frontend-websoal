    ...
    init: (formId, selectId, inputBaruId, containerBaruId, tabelId, hasilId, paginationId) => {
      ...
      const renderTable = () => {
        ...
        if (pageData.length === 0) {
          tabel.innerHTML = `<tr><td colspan="4" class="text-center py-4 text-gray-500">Belum ada data.</td></tr>`;
      ...
      // 🔹 Load data awal (materi dan description)
      ...
      // 🔹 Event: ketika dropdown materi diubah
      ...
      form.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          let idMateri;
          // jika pilih "materi baru", tambahkan dulu ke API
          if (selectedId === "new") {
            const materiRes = await MateriAPI.add(namaMateriBaru.value.trim());
            if (!materiRes.id_materi) throw new Error("Gagal menambahkan materi baru.");
            idMateri = materiRes.id_materi;
          } else {
